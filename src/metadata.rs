...
impl ::serde::ser::Serialize for RatioWrapper {
    fn serialize<S>(&self, serializer: &mut S) -> Result<(), S::Error> where S: serde::Serializer {
        let t = (self.0.numer(), self.0.denom());
        serializer.visit_tuple(serde::ser::impls::TupleVisitor2::new(&t))
    }
}

// partially generated by serde_codegen for a similar type
impl serde::Deserialize for RatioWrapper {
    fn deserialize<D>(deserializer: &mut D) -> Result<RatioWrapper, D::Error>
        where D: serde::Deserializer
    {
        struct Visitor<D: serde::Deserializer>(::std::marker::PhantomData<D>);
        impl <D: ::serde::de::Deserializer> ::serde::de::Visitor for Visitor<D> {
            type Value = RatioWrapper;
            #[inline]
            fn visit_seq<V>(&mut self, mut visitor: V) -> Result<RatioWrapper, V::Error>
                where V: serde::de::SeqVisitor
            {
                ...
                try!(visitor.end());
                Ok(RatioWrapper(num::rational::Ratio::<i32>::new_raw(field0, field1)))
            }
        }
        deserializer.visit_tuple_struct("Ratio", 2usize, Visitor::<D>(::std::marker::PhantomData))
    }
}
        ...
        impl serde::de::Visitor for OrientationVisitor {
                ...
                where E: serde::de::Error
            {
                match value {
                   ...
                   _ => Err(serde::de::Error::syntax("unexpected value for orientation")),
                }
            }
        }

        deserializer.visit(OrientationVisitor)
    }
}
...
impl Serialize for OrientationWrapper {
    fn serialize<S>(&self, serializer: &mut S) -> Result<(), S::Error> where S: Serializer {
        serializer.visit_str(orientation_to_str(self.0))
    }
}
        ...
        Some(Metadata {
            ...
            exposure_time: match exif.as_ref() {
                Some(ref e) => match e.get_exposure_time() {
                    Some(exposure) => Some(RatioWrapper(exposure)),
                    None => None,
                },
                None => None,
            },
            ...
            fnumber: match exif.as_ref() {
                Some(ref e) => {
                    ...
                    match ratio {
                        Some(fnumber) => Some(RatioWrapper(fnumber)),
                        None => None,
                    }
                }
                None => None
